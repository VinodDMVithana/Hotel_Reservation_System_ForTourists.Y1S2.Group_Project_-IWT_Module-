            ...
            <table>
                ...
                <tbody>
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete'])) {
                        ...
                        if ($stmt->execute()) {
                            echo '<script>alert("User deleted successfully."); window.location.href = "ViewAllUser.php";</script>';
                        } else {
                            echo '<script>alert("Error deleting user.");</script>';
                        }
                    }
                    $acc_sql = "SELECT * FROM user";
                        ...
                        if ($acc_result->num_rows > 0) {
                            while ($acc_row = $acc_result->fetch_assoc()) { ?>
                                <tr>
                                    ...
                                    <td><?php echo $acc_row['email']; ?></td>
                                    <td><?php echo $acc_row['mobilenumber']; ?></td>
                                    <td><?php echo $acc_row['country']; ?></td>
                                    <td><?php echo ($acc_row['usertype'] == 1) ? 'User' : 'Manager'; ?></td>
                                    <td class="action-buttons">
                                        <a href="./UpdateUser.php?userID=<?php echo $acc_row['userid']; ?>">
                                            <button type="submit" value="editBtn"><img src="../../assets/edit-text.png" alt="edit" width="16" height="16"></button>
                                        </a>
                                        <form method="post" onsubmit="return confirm('Are you sure you want to delete this user?');" style="margin-top: 10px;">
                                            <input type="hidden" name="delete" value="<?php echo $acc_row['userid']; ?>">
